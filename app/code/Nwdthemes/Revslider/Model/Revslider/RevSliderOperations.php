...
use \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderBaseAdmin;
...
class RevSliderOperations extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderElementsBase {
...
public static function parseCustomAnimationByArray($animArray, $is = 'start', $frame_val){
	...
	public function getArrCaptionClasses($contentCSS){
		...
		$parser = new RevSliderCssParser($this->_framework, $this->_query, $this->_resource);
	...
	public function getArrFontFamilys() {
		if ( ! $fonts = $this->_registerHelper->getFromRegister('fonts', 'google')) {
            ...
            $googlefonts = $this->_googleFonts->getFonts();
			...
			$fonts = $this->_framework->apply_filters('revslider_operations_getArrFontFamilys', $fonts);

            $this->_registerHelper->setToRegister('fonts', 'google', $fonts);
		}
		return $fonts;
	}
	...
	public static function getStaticCss(){
		if(!self::$framework->get_option('revslider-static-css')){
			if(file_exists(Framework::$RS_PLUGIN_PATH.'public/assets/css/static-captions.css')){
				$contentCSS = @file_get_contents(Framework::$RS_PLUGIN_PATH.'public/assets/css/static-captions.css');
...
*/
	public static function updateStaticCss($content){
		...
		$c = self::$framework->get_option('revslider-static-css', '');
	...
	public function getDynamicCss(){
		...
		$styles = RevSliderCssParser::parseDbArrayToCss(self::$css, "\n");
		return $styles;
	}
	...
	public static function getCaptionsCssContentArray(){
		if(file_exists(Framework::$RS_PLUGIN_PATH.'public/assets/css/captions.css'))
			$contentCSS = file_get_contents(Framework::$RS_PLUGIN_PATH.'public/assets/css/captions.css');
		else if(file_exists(Framework::$RS_PLUGIN_PATH.'public/assets/css/captions-original.css'))
			$contentCSS = file_get_contents(Framework::$RS_PLUGIN_PATH.'public/assets/css/captions-original.css');
		else if(file_exists(Framework::$RS_PLUGIN_PATH.'backup/'.'captions.css'))
			$contentCSS = file_get_contents(Framework::$RS_PLUGIN_PATH.'backup/'.'captions.css');
		else if(file_exists(Framework::$RS_PLUGIN_PATH.'backup/'.'captions-original.css'))
			$contentCSS = file_get_contents(Framework::$RS_PLUGIN_PATH.'backup/'.'captions-original.css');
		else
			$contentCSS = '';
	...
	public function previewOutput($sliderID,$output = null, $storeID = 0) {
		...
		if($output == null)
			$output = new RevSliderOutput($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
		...
		//put the output html
		$urlPlugin = Framework::$RS_PLUGIN_URL.'public/assets/';
		...
		$setBase = ($this->_framework->is_ssl()) ? "https://" : "http://";
        ...
        $http = ($this->_framework->is_ssl()) ? 'https' : 'http';
		...
		foreach($gf as $gfk => $gfv){
			...
			if(!empty($gfv['subsets'])){
				foreach($gfv['subsets'] as $ssk => $ssv){
					if(array_search($this->_framework->esc_attr($gfk.'+'.$ssv), $gfsubsets) !== false){
						$subsets[] = $ssv;
					}
				}
			}
        ...
        $this->_framework->do_action('revslider_preview_slider_head');
        $output->putSliderBase($sliderID);
        $this->_framework->do_action('revslider_preview_slider_footer', $slider, $output);
	}
	...
	<head>
        ...
        foreach ($this->_framework->getRegisteredStyles() as $_handle => $_style) : ?>
            <link  rel="stylesheet" type="text/css"  media="all" href="<?php echo $_style; ?>" />
        <?php endforeach; ?>

		<link rel="stylesheet" type="text/css" href="<?php echo $path_fonts; ?>pe-icon-7-stroke/css/pe-icon-7-stroke.min.css">
		...
		if($export_real){
			...
			?>
			<link rel="stylesheet" type="text/css" href="<?php echo $path_css; ?>layers.min.css">

			<!-- REVOLUTION NAVIGATION STYLES -->
			<link rel="stylesheet" type="text/css" href="<?php echo $path_css; ?>navigation.min.css">
        ...
        echo $this->_framework->getLocalizeScriptsHtml();
...
ob_end_clean();
        ...
        $search = $this->_framework->apply_filters('revslider_html_export_replace_urls', $search);
			...
			if(!empty($_files) && isset($_files[0]) && !empty($_files[0])){
				...
				foreach($_files[0] as $_file){
					...
					if($f == false){
						...
						if(strpos($o, 'public/assets/js/') !== false && strpos($o, Plugin::WP_PLUGIN_DIR) === false) { //this will be the jsFileLocation script part
        ...
        foreach ($dotOverlays as $dotClass => $dotFile) {
            if (strpos($slider_html, 'dottedOverlay:"' . $dotClass . '"') !== false) {
                if( ! $usepcl) {
                    $zip->addFile(Framework::$RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, $path_assets_raw.'/'.$dotFile);
                }else{
                    $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/', $path_assets_raw.'/');
                }
            }
        }
		...
		if($export_real){ //only include if real export
			...
			if(!$usepcl){
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.actions.min.js', 'js/extensions/revolution.extension.actions.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.carousel.min.js', 'js/extensions/revolution.extension.carousel.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.kenburn.min.js', 'js/extensions/revolution.extension.kenburn.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.layeranimation.min.js', 'js/extensions/revolution.extension.layeranimation.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.migration.min.js', 'js/extensions/revolution.extension.migration.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.navigation.min.js', 'js/extensions/revolution.extension.navigation.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.parallax.min.js', 'js/extensions/revolution.extension.parallax.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.slideanims.min.js', 'js/extensions/revolution.extension.slideanims.min.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.video.min.js', 'js/extensions/revolution.extension.video.min.js');

				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/export/js/jquery-migrate.js', 'js/jquery-migrate.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/export/js/jquery.js', 'js/jquery.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/export/js/require.js', 'js/require.js');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/export/js/requirejs-config.js', 'js/requirejs-config.js');

                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/TouchSwipe.js', 'js/TouchSwipe.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/themepunch_gs.js', 'js/themepunch_gs.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/TweenLite.js', 'js/TweenLite.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/TimelineLite.js', 'js/TimelineLite.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/easing/EasePack.js', 'js/EasePack.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/CSSPlugin.js', 'js/CSSPlugin.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/SplitText.js', 'js/SplitText.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/tools/waitForImages.js', 'js/waitForImages.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/jquery.themepunch.tools.min.js', 'js/jquery.themepunch.tools.min.js');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/js/jquery.themepunch.revolution.min.js', 'js/jquery.themepunch.revolution.min.js');

				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/css/settings.css', 'css/settings.css');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/css/openhand.cur', 'css/openhand.cur');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/css/closedhand.cur', 'css/closedhand.cur');

				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.min.css', 'fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.min.css');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/css/helper.min.css', 'fonts/pe-icon-7-stroke/css/helper.min.css');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff');

				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/css/font-awesome.min.css', 'fonts/font-awesome/css/font-awesome.min.css');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/FontAwesome.otf', 'fonts/font-awesome/fonts/FontAwesome.otf');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.eot', 'fonts/font-awesome/fonts/fontawesome-webfont.eot');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.svg', 'fonts/font-awesome/fonts/fontawesome-webfont.svg');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.ttf', 'fonts/font-awesome/fonts/fontawesome-webfont.ttf');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff', 'fonts/font-awesome/fonts/fontawesome-webfont.woff');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', 'fonts/font-awesome/fonts/fontawesome-webfont.woff2');

				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.eot', 'fonts/revicons/revicons.eot');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.svg', 'fonts/revicons/revicons.svg');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.ttf', 'fonts/revicons/revicons.ttf');
				$zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.woff', 'fonts/revicons/revicons.woff');

                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/assets/coloredbg.png', 'assets/coloredbg.png');
                $zip->addFile(Framework::$RS_PLUGIN_PATH.'/public/assets/assets/loader.gif', 'assets/loader.gif');
			}else{
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.actions.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.carousel.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.kenburn.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.layeranimation.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.migration.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.navigation.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.parallax.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.slideanims.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.video.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');

				$pclzip->add(Framework::$RS_PLUGIN_PATH.'export/js/jquery-migrate.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'export/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'export/js/jquery.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'export/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'export/js/require.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'export/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'export/js/requirejs-config.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'export/js/', PCLZIP_OPT_ADD_PATH, 'js/');

                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/TouchSwipe.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/themepunch_gs.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/TweenLite.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/TimelineLite.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/easing/EasePack.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/easing/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/CSSPlugin.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/SplitText.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/waitForImages.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/tools/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/jquery.themepunch.tools.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/js/jquery.themepunch.revolution.min.js', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');

				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/css/settings.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/css/openhand.cur', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/css/closedhand.cur', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');

				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.min.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/helper.min.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');

				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/css/font-awesome.min.css', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/FontAwesome.otf', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.eot', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.svg', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.ttf', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');

				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.eot', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.svg', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.ttf', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.woff', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');

                $pclzip->add(Framework::$RS_PLUGIN_PATH.'/public/assets/assets/coloredbg.png', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Framework::$RS_PLUGIN_PATH.'/public/assets/assets/loader.gif', PCLZIP_OPT_REMOVE_PATH, Framework::$RS_PLUGIN_PATH.'public/assets/');
			}
		...
		header("Content-Disposition: attachment; filename=".$this->_framework->sanitize_title($slider->getAlias()).".zip");
	...
	public function loadingMessageOutput(){
		?>
		<div class="message_loading_preview"><?php echo __( "Loading Preview...")?></div>
		<?php
	}
	...
	public function updateGeneralSettings($data){
		...
		$this->_framework->update_option('revslider-global-settings', $data);
	...
	static function getGeneralSettingsValues(){
		...
		$arrValues = self::$framework->get_option('revslider-global-settings', '');

		$arrValues = self::$framework->maybe_unserialize($arrValues);
	...
	public static function deleteGoogleFonts(){
		self::$framework->update_option('rs_google_font', time());
	}
	...
	public static function printCleanFontImport(){
        ...
        if($dl_fonts === 'preload'){
            if(!empty($fonts)){
                $upload_dir   = self::$framework->wp_upload_dir();
                $base_dir     = $upload_dir['basedir'];
                $base_url     = $upload_dir['baseurl'];
                $rs_google_ts = self::$framework->get_option('rs_google_font', 0);

                foreach($fonts as $key => $font){
                    //check if we downloaded the font already
                    $font_name = preg_replace('/[^-a-z0-9 ]+/i', '', $key);
                    $font_name = strtolower(str_replace(' ', '-', self::$framework->esc_attr($font_name)));
                    ...
                    if(!is_file($base_dir . $fontPath . $font_name . '/' . $font_name . '.woff2') || filemtime($base_dir . $fontPath . $font_name . '/' . $font_name . '.woff2') < $rs_google_ts){
                        ...
                        $content    = self::$framework->wp_remote_get($url);
                        ...
                        if(preg_match_all($regex_url, $body, $found_fonts)){
                            foreach($found_fonts as $found_font){
                                ...
                                $f_c = self::$framework->wp_remote_get($found_font);
...
"@font-face {
  ...
  src: local('".$f_family."'), local('".$f_family."'), url(".$base_url . $fontPath . $font_name . '/' . $font_name . '.woff2'.") format('woff2');
        ...
        return self::$framework->apply_filters('revslider_printCleanFontImport', $ret);
    }
	...
	public static function getCleanFontImport($font, $class = '', $url = '', $variants = array(), $subsets = array()){
        ...
        return self::$framework->apply_filters('revslider_getCleanFontImport', $ret, $font, $class, $url, $variants, $subsets);
	}
	...
	public function checkPurchaseVerification($data){
        ...
        do {
            ...
            $response = $this->_framework->wp_remote_post($url . '/activate.php', array(
                'body' => array(
		...
		if($version_info == 'valid'){
			...
			$this->_framework->update_option('revslider-code', $data['code']);
            $this->_framework->update_option('revslider-temp-active-notice', 'false');
			return true;
            ...
            $this->_framework->update_option('revslider-valid', 'true');
            $this->_framework->update_option('revslider-code', $data['code']);
            $this->_framework->update_option('revslider-temp-active', 'true');
            $this->_framework->update_option('revslider-temp-active-notice', 'false');
            return 'temp';
		}else{
			return false;
	...
	public function doPurchaseDeactivation($data){
        ...
        do {
            $url = $this->_revSliderLoadBalancer->get_url('updates');
            $response = $this->_framework->wp_remote_post($url . '/deactivate.php', array(
                'body' => array(
                    'code' => urlencode($code),
                    'product' => urlencode(RevSliderGlobals::PLUGIN_SLUG)
                )
		...
		if($version_info == 'valid'){
            ...
            $this->_framework->update_option('revslider-code', '');
			return true;
		}else{
			return false;
		}
	...
	public static function get_slider_speed($sliderID){
		...
		$do_ssl = (self::$framework->is_ssl()) ? 'http:' : 'https:';
			...
			foreach($slides as $key => $slide){
				...
				if(isset($slide['params']['image']) && $slide['params']['image'] != ''){
					...
					if(isset($slide['params']['image_id'])){
						...
						$urlImage = self::$images->wp_get_attachment_image_src($slide['params']['image_id'], $slide['params']['image_source_type']);
					}
					if($urlImage === false){
						$cur_img_id = RevSliderFunctionsWP::get_image_id_by_url($slide['params']['image']);
						if($cur_img_id !== false){
							$urlImage = self::$images->wp_get_attachment_image_src($cur_img_id, $slide['params']['image_source_type']);
						}
					}

					if($urlImage !== false){
						$infos['id'] = $cur_img_id;
						$file = self::$images->get_attached_file( $cur_img_id );
						$infos['info'] = pathinfo( $file );

						if(file_exists( $file )){
							$infos['size'] = filesize( $file );
							$infos['size-format'] = self::$framework->size_format($infos['size'], 2);
							$img_size += $infos['size'];
							if ($slide_counter==1)
							...
							case 'image':
								...
								if(isset($layer['image_url']) && trim($layer['image_url']) != ''){
									...
									if($cur_img_id !== false){
										...
										$urlImage = self::$images->wp_get_attachment_image_src($cur_img_id, $layer['layer-image-size']);
											...
											case 'html5':
												...
												if(!empty($video_arr)){
													foreach($video_arr as $type => $url){
														...
														if($cur_id !== false){
															$infos['id'] = $cur_id;
															$file = self::$images->get_attached_file( $cur_id );
															$infos['info'] = pathinfo( $file );
															if(file_exists( $file )){
																$infos['size'] = filesize( $file );
																$infos['size-format'] = self::$framework->size_format($infos['size'], 2);
		...
		if(!empty($used_images)){
			...
			<span style="float:left;width:40%">
				...
				<span id="image_sub_size" class="tp-monitor-total-subsize"><?php echo self::$framework->size_format($img_size,2); ?></span>
				</span>
				...
				foreach($used_images as $path => $image){
					...
					if(isset($image['size'])) {
						...
						if ($image['size']>1000000)
							$_li .=   '<span class="tp-monitor-size">'.self::$framework->size_format($image['size'],2).'</span>';
						else
							$_li .=   '<span class="tp-monitor-size">'.self::$framework->size_format($image['size'],0).'</span>';
					}else{
		...
		?>
			...
			<span style="float:left;width:40%; display:block">
				...
				<?php if ($video_size>0) {?>
					<span class="tp-monitor-imageicon"></span>
					<span id="video_sub_size" class="tp-monitor-total-subsize"><?php echo self::$framework->size_format($video_size,2); ?></span>
				<?php } else {?>
					<span class="tp-monitor-imageicon"></span>
			...
			foreach($used_videos as $path => $video){
				...
				if(isset($video['size'])){
					...
					if ($video['size']>1000000)
						$_li .= '<span class="tp-monitor-size">'.self::$framework->size_format($video['size'],2).'</span>';
					else
						$_li .= '<span class="tp-monitor-size">'.self::$framework->size_format($video['size'],0).'</span>';
				}else{
		...
		if(file_exists( Framework::$RS_PLUGIN_PATH . '/public/assets/css/settings.css' )){
			$fs = filesize( Framework::$RS_PLUGIN_PATH . '/public/assets/css/settings.css' );
			$_li ='<li class="tp-monitor-listli">';
			if ($fs<60000)
				$_li .= '<span class="tp-monitor-good"></span>';
			else
			...
			$_li .= '<span class="tp-monitor-size">'.self::$framework->size_format($fs,0).'</span>';
			$_li .= '<span class="tp-monitor-file">';
			$_li .= 'css/settings.css';
			$_li .= '</span>';
		...
		$_li .= '<span class="tp-monitor-size">'.self::$framework->size_format(strlen($custom_css),0).'</span>';
		$_li .= '<span class="tp-monitor-file">';
		...
		*/
		...
		if(!empty($used_captions)){
			...
			$_li .=  '<span class="tp-monitor-size">'.self::$framework->size_format(strlen($styles),0).'</span>';
			$_li .=  '<span class="tp-monitor-file">';
		...
		echo '</ul>';
		echo ' <span style="display:none" id="css-size-hidden">'.self::$framework->size_format($css_size, 2).'</span>';
		...
		$jsfiles = array(
			'jquery.themepunch.tools.min.js' => Framework::$RS_PLUGIN_PATH . '/public/assets/js/jquery.themepunch.tools.min.js',
			'jquery.themepunch.revolution.min.js' => Framework::$RS_PLUGIN_PATH . '/public/assets/js/jquery.themepunch.revolution.min.js',

		);
		//check which js files will be used by the Slider
		if($using_kenburns == true) $jsfiles['revolution.extension.kenburn.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.kenburn.min.js';
		if($using_parallax == true) $jsfiles['revolution.extension.parallax.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.parallax.js';
		if($using_navigation == true) $jsfiles['revolution.extension.navigation.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.navigation.min.js';
		if($using_videos == true) $jsfiles['revolution.extension.video.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.video.min.js';
		if($using_actions == true) $jsfiles['revolution.extension.actions.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.actions.min.js';
		if($using_layeranim == true) $jsfiles['revolution.extension.layeranimation.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.layeranimation.min.js';
		if($using_carousel == true)
			$jsfiles['revolution.extension.carousel.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.carousel.min.js';
		else
			$jsfiles['revolution.extension.slideanims.min.js'] = Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.slideanims.min.js';
		...
		foreach($jsfiles as $name => $path){
			if(file_exists( $path )){
				...
				echo '<span class="tp-monitor-size">'.self::$framework->size_format($fs,0).'</span>';
		...
		echo '</ul>';
		echo ' <span style="display:none" id="css-size-hidden">'.self::$framework->size_format($js_size, 2).'</span>';


		$http = (self::$framework->is_ssl()) ? 'https' : 'http';
		...
		$content = str_replace("%overall_size%",self::$framework->size_format($total_size,2),$content);
		...
		$content = str_replace("%css_size%",self::$framework->size_format($css_size,2),$content);

		$content = str_replace("%js_performance%",$js["proc"],$content);
		$content = str_replace("%js_color%",$js["col"],$content);
		$content = str_replace("%js_size%",self::$framework->size_format($js_size,2),$content);
		...
		$content = str_replace("%firstslide_size%", self::$framework->size_format($firstslide_size,2), $content);
		$content = str_replace("%smartslide_size%", self::$framework->size_format($smartslide_size,2), $content);
		$content = str_replace("%allslide_size%", self::$framework->size_format($total_size,2), $content);
...
$presets[] = array (
    'settings' => array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Auto', 'preset' => 'standardpreset' ),
    'values' =>
    array (
...
$presets[] = array (
    'settings' =>
    array ( 'class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Full-Width', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Full-Screen', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/thumb_auto1.png', 'name' => 'Thumbs-Bottom-Auto', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/thumbs_left_auto.png', 'name' => 'Thumbs-Left-Auto', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/thumbs_right_auto.png', 'name' => 'Thumbs-Right-Auto', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/scroll_fullscreen.png', 'name' => 'Vertical-Bullet-Full-Screen', 'preset' => 'standardpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Wide-Full-Screen', 'preset' => 'heropreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Wide-Full-Width', 'preset' => 'heropreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Regular-Full-Screen', 'preset' => 'heropreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Regular-Full-Width', 'preset' => 'heropreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/cover_carousel_thumbs.png', 'name' => 'Cover-Flow-Thumbs', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/cover_carousel_endless.png', 'name' => 'Cover-Flow-Infinite', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/flat_carousel_thumbs.png', 'name' => 'Flat-Infinite-Thumbs', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/flat_carousel.png', 'name' => 'Flat-Infinite', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/flat_carousel_thumbs_left.png', 'name' => 'Flat-Thumbs-Left', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/carousel_thumbs_right_fullscreen.png', 'name' => 'Full-Screen-Thumbs-Right', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/cover_carousel_thumbs.png', 'name' => 'Cover-Flow-Full-Screen', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' =>
    array ('class' => '', 'image' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sliderpresets/carousel_full_rounded.png', 'name' => 'Cover-Flow-Rounded', 'preset' => 'carouselpreset' ),
    'values' =>
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
	...
	//data.layer - Layer as jQuery Object
      ...
      'custom_javascript' => '',
		...
		$customer_presets = self::$framework->get_option('revslider_presets', array());


		$presets = array_merge($presets, $customer_presets);

		$presets = self::$framework->apply_filters('revslider_slider_presets', $presets);

		foreach($presets as $key => $preset){
			if(intval($preset['settings']['image']) > 0){
				$img = self::$images->wp_get_attachment_image_src(self::$framework->esc_attr($preset['settings']['image']), 'medium');
	...
	public static function create_slider_page($added){
		...
		if(!is_array($added)) return self::$framework->apply_filters('revslider_create_slider_page', $new_page_id, $added);

		$content = '';
		$page_id = self::$framework->get_option('rs_import_page_id', 1);
		...
		foreach($added as $sid){
			...
			if($alias !== ''){
                $content .= '{{block class="Nwdthemes\Revslider\Block\Revslider" alias="'.$alias.'"}}' . "\n"; //this way we will reorder as last comes first
			}
		}

		if($content !== '') {

            $_title = __('Revolution Slider Page') . ' - ' . self::$framework->sanitize_title($slider->getTitle());
            $_id = 'revslider-' . $page_id . '-' . time() . '-' . self::$framework->sanitize_title_with_dashes($alias);

            $pageUrl = self::$framework->createPage($_id, $_title, $content);

            if ($pageUrl) {
                $page_id++;
                self::$framework->update_option('rs_import_page_id', $page_id);
            }
		}

		return self::$framework->apply_filters('revslider_create_slider_page', $pageUrl, $added);
	}
	...
	public function get_installed_cache_plugins(){
		//get all plugins
		$plugins = $this->_framework->get_plugins();
		...
		return $this->_framework->apply_filters('revslider_get_installed_cache_plugins', $all);
	...
	public static function import_failed_message($message, $link = false){
		...
		echo '<div style="font-family:arial; width:100%;height:100%;position:absolute;top:0px;left:0px;background-image:url('.Framework::$RS_PLUGIN_URL.'admin/assets/images/errorbg.jpg); background-position:center center; background-size:cover;">';
