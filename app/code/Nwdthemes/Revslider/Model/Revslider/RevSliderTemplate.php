...
class RevSliderTemplate {
	...
	public function _download_template($uid){
        ...
        $return = false;
		$uid = $this->_framework->esc_attr($uid);

		$code = ($this->_framework->get_option('revslider-valid', 'false') == 'false') ? '' : $this->_framework->get_option('revslider-code', '');

		$upload_dir = $this->_framework->wp_upload_dir(); // Set upload folder
		// Check folder permission and define file location
		if($this->_filesystem->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) { //check here to not flood the server
            $done    = false;
            $count    = 0;
            do{
                $url        = $this->_revSliderLoadBalancer->get_url('templates');
                $request    = $this->_framework->wp_remote_post($url.'/'.$this->templates_download, array(
				'body' => array(
					'code' => urlencode($code),
	...
	public function _delete_template($uid){
		...
		// Check folder permission and define file location
		if( $this->_filesystem->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) {
			$file = $upload_dir['basedir']. $this->templates_path . '/' . $uid.'.zip';
	...
	public function _get_template_list($force = false){
		...
		$last_check = $this->_framework->get_option('revslider-templates-check');

		if($last_check == false){ //first time called
			$last_check = 172801;
			$this->_framework->update_option('revslider-templates-check',  time());
		}
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
            ...
            do{
                $url        = $this->_revSliderLoadBalancer->get_url('templates');
                $request    = $this->_framework->wp_remote_post($url.'/'.$this->templates_list, array(
				'body' => array(
					'code' => urlencode($code),
					'shop_version' => urlencode(self::SHOP_VERSION),
					'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
	...
	private function update_template_list(){
		...
		$new = $this->_framework->get_option('rs-templates-new', false);
		$cur = $this->_framework->get_option('rs-templates', array());
        $cur = array();
	...
	private function _update_images($firstLevelOnly = false){
		...
		$templates = $this->_framework->get_option('rs-templates', array());
        $chk = $this->check_curl_connection();

        $curl = ($chk) ? $this->_curl : false;
        $url        = $this->_revSliderLoadBalancer->get_url('templates');
			...
			if(!empty($templates['slider']) && is_array($templates['slider'])){
				foreach($templates['slider'] as $key => $temp){
					...
					if( $this->_filesystem->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) {
						...
						$file_plugin = Framework::$RS_PLUGIN_PATH . $this->templates_path_plugin . '/' . $temp['img'];
						...
						if((!file_exists($file) && !file_exists($file_plugin)) || isset($temp['push_image'])){
                            if($curl !== false){
                                ...
                                if(!$this->_framework->is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
                                ...
                                $options = array(
                                    'http'=>array(
                                        ...
                                        "Cookie: foo=bar\r\n" .
                                        'User-Agent: ' . $curl->getUserAgent() . ' - fgc - '.$count."\r\n"
                                    )
                                );
				...
				foreach(array_reverse($templates['slides']) as $key => $temp){
					foreach($temp as $k => $tvalues){
						...
						if( $this->_filesystem->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) {
							...
							$file_plugin = Framework::$RS_PLUGIN_PATH . $this->templates_path_plugin . '/' . $tvalues['img'];
	...
	public function copySlideToTemplates($slide_id, $slide_title, $slide_settings = array()){
		...
		$slide_title = $this->_framework->sanitize_text_field($slide_title);
		...
		$duplicate = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %s", $slide_id), Query::ARRAY_A);
	...
	public function getTemplateSlides(){
		...
		$templates = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", -1), Query::ARRAY_A);
	...
	private function getDefaultTemplateSlides(){
		...
		$templates = $this->_framework->apply_filters('revslider_set_template_slides', $templates);
		...
		if(!empty($sliders)){
			foreach($sliders as $slider){
				...
				if(!isset($slider['installed'])){
					$templates = array_merge($templates, $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", $slider['id']), Query::ARRAY_A));
				}else{
	...
	public function getDefaultTemplateSliders(){
		...
		$check = $wpdb->get_results("SELECT * FROM $table_name WHERE type = 'template'", Query::ARRAY_A);

		$sliders = $this->_framework->apply_filters('revslider_set_template_sliders', $sliders);
	...
	public function getThemePunchTemplateSliders(){
		...
		//add themepunch default Sliders here
		$sliders = $wpdb->get_results("SELECT * FROM $table_name WHERE type = 'template'", Query::ARRAY_A);

		$defaults = $this->_framework->get_option('rs-templates', array());
	...
	public function _check_file_path($image, $url = false){
		...
		$file_plugin = Framework::$RS_PLUGIN_PATH . $this->templates_path_plugin . '/' . $image;
		...
		if(file_exists($file)){ //downloaded image first, for update reasons
			...
			if($url){
				$image = Framework::$RS_PLUGIN_URL . $this->templates_path_plugin . '/' . $image;
			}else{
				$image = Framework::$RS_PLUGIN_URL . $this->templates_path_plugin . '/' . $image;
	...
	public function write_import_template_markup($template){
		...
		if($template['img'] == ''){
			$template['img'] = Framework::$RS_PLUGIN_URL . self::DEFAULT_IMAGE;
		}
        ...
        <div data-src="<?php echo $template['img']; ?>" class="template_slider_item_import"
			...
			data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"
			<?php
            ...
            <div class="template_preview_add_wrapper">
                ...
                <a class="preview_template_slider" href="<?php echo $this->_framework->esc_attr($template['preview']); ?>" target="_blank"><i class="eg-icon-search"></i></a>
                <?php } ?>
        ...
        <div class="template_thumb_more">
            ...
            <ul class="ttm_requirements">
                ...
                if(isset($template['plugin_require']) && !empty($template['plugin_require'])){
                    foreach($template['plugin_require'] as $pk => $pr){
                        ...
                        if(isset($pr['url'])) echo '<a href="'.$this->_framework->esc_attr($pr['url']).'" target="_blank">';
                        echo $pr['name'];
            ...
            if($deny == '' && $allow_install == true){
                if(isset($template['package_parent']) && $template['package_parent'] !== ''){
                    ...
                    ?>
                    <div class="install_template_slider<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider'); ?></div>
                    <?php
                }
                if(isset($template['package']) && $template['package'] !== ''){
                    ?>
                    <span class="tp-clearfix" style="margin-bottom:5px"></span>
                    <div class="install_template_slider_package<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider Pack'); ?></div>
                    <?php
                }
            } else {
                ?>
	...
	public function write_import_template_markup_slide($template){
        ...
        <div class="template_thumb_more">
            ...
            if($deny == '' && $allow_install == true){
                ?>
                <div class="install_template_slide<?php echo $deny; ?>" data-slidenumber="<?php echo $template['number']; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider'); ?></div>
                <?php
            } else {
                ?>
                <div class="dontadd_template_slider_item"><i class="icon-not-registered"></i><?php echo __('Requirements not met'); ?></div>
            ...
            <div class="template_thumb_more">
                ...
                if ($allow_install !== false) {
                    ...
                    if ($slider_id !== false && isset($template['package']) && $template['package'] !== '' && $template['package_full_installded']) {
                        ?>
						<div class="add_template_slider_item_package" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-plus"></i><?php echo __('Add Slider Pack'); ?></div>
                        <?php
                    }
                    if($slider_id !== false){
                        ?>
                        <span class="tp-clearfix" style="margin-bottom:5px"></span>
                        <div class="install_template_slider" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Re-Download Slider'); ?></div>
                        <span class="tp-clearfix" style="margin-bottom:5px"></span>
                        <?php
                        if(isset($template['package']) && $template['package'] !== ''){
                            $txt = ($template['package_full_installded']) ? __('Re-Download Slider Pack') : __('Install Slider Pack');
                            ?>
                            <div class="install_template_slider_package" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo $this->_framework->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo $txt; ?></div>
                            <span class="tp-clearfix" style="margin-bottom:5px"></span>
                            <?php
                        }
                    }
    ...
    public function create_html_slides($tp_template_slider, $all_slider, $templates){
                    ...
                    if(is_array($m_slider['plugin_require']) && !empty($m_slider['plugin_require'])){
                        foreach($m_slider['plugin_require'] as $k => $pr){
                            if(!isset($plugin_list[$pr['path']])){
                                if($this->_framework->is_plugin_active($this->_framework->esc_attr($pr['path']))){
                    ...
                    if(!empty($c_slides)){
                        ...
                        <div style="margin-bottom:30px" class="template_group_wrappers  <?php echo $slidercat.$slidercat_new.' '; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">
                            ...
                            if(isset($m_slider['preview']) && $m_slider['preview'] !== ''){
                                echo '<a class="template_slide_preview" href="'.$this->_framework->esc_attr($m_slider['preview']).'" target="_blank"><i class="eg-icon-search"></i></a>';
                            }
                            ...
                            foreach($c_slides as $key => $c_slide){
                                ...
                                <div class="temp_slide_single_wrapper">
                                    ...
                                    <div class="template_meta_line">
                                        <?php if ($isnew) { ?>
                                        <span class="template_new"><?php echo __("New", "revslider"); ?></span>
                                        <?php } ?>
                                        <?php if ($isupdate) { ?>
                                        <span class="template_new"><?php echo __("Update", "revslider"); ?></span>
                                        <?php } ?>
                                        <span class="<?php echo $slidercat;?>"><?php echo __($etikett_a, "revslider");?></span>
                                        <span class="template_installed"><?php echo __("Installed", "revslider"); ?><i class="eg-icon-check"></i></span>
                                    </div>
                            ...
                            foreach($c_slides as $key => $c_slide){
                                ...
                                <?php
                                    ...
                                    <div class="template_meta_line">
                                        ...
                                        <?php }*/ ?>
                                        <span class="<?php echo $slidercat;?>"><?php echo __($etikett_a, "revslider");?></span>
                                        <span class="template_notinstalled"><?php echo __("Not Installed", "revslider"); ?></span>
                                    </div>
                    ...
                    <div class="template_group_wrappers temp_existing <?php //if(isset($c_slider['filter'])){ echo implode(' ', $c_slider['filter']); } ?>">
                        ...
                        foreach($c_slides as $c_slide){
                            ...
                            <?php
                                ...
                                ?>
                                <div class="template_meta_line">
                                    <span class="template_local"><?php echo __("Local Slide", "revslider");?></span>
                                </div>
                                <div class="template_thumb_title"><?php echo $title; ?></div>
                            </div>
                            <?php
        ...
        <div class="template_group_wrappers temp_custom">
            ...
            if(!empty($templates)){
                ?>
                <div class="template_slider_title"><?php echo __('User Templates'); ?></div>
                <div class="temp_slides_in_slider_wrapper">
                <?php
                foreach($templates as $template){
                    ...
                    <div class="temp_slide_single_wrapper">
                        ...
                        ?>
                        <div class="template_meta_line">
                            <span class="template_local"><?php echo __("User Template", "revslider");?></span>
                        </div>
                        <div class="template_thumb_title"><?php echo $title; ?></div>
                    </div>
                    <?php
    ...
    public function create_html_sliders($tp_template_slider){
        ...
        if(!empty($tp_template_slider)){
				...
				if($is_package_parent){
					...
					if(!empty($uids)){
						foreach($uids as $sid => $uid){
							...
							slider_package_names["<?php echo $uid; ?>"].name = "<?php echo $this->_framework->esc_html($spt); ?>";
            ...
            echo __('No data could be retrieved from the servers. Please make sure that your website can connect to the themepunch servers.');
            echo '</span>';
        }
        ?>
