...
class RevSliderOutput {
    ...
    public static function putSlider($sliderID,$putIn="", $gal_ids = array(), $settings = array(), $order = array()){
        $settings = self::$framework->apply_filters('revslider_modify_slider_settings', $settings, $sliderID);

		//check if on mobile and if option hide on mobile is set

		$output = new RevSliderOutput(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts, self::$registerHelper);
	...
	private function filterOneSlide($slides){
        ...
        if(strpos($oneSlideID, 'static_') !== false){
            ...
            $tmp_slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".RevSliderGlobals::$table_static_slides." WHERE slider_id = %s", array($sliderID)), Query::ARRAY_A);

            if(!empty($tmp_slides)){
                $n_slides = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
    ...
    private function putSlides($gal_ids = array(), $order = array()){
		...
		if($this->previewMode == true && $this->oneSlideMode == true){
			...
			$previewSlide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
            ...
            $slides = $this->slider->getSlidesForOutput($publishedOnly, $this->previewMode ? $this->storeID : $this->_framework->getStoreId(), $this->sliderLang, $gal_ids);
		...
		if(empty($slides)){
			...
			<div class="no-slides-text">
				...
				if($this->slider->isSlidesFromPosts()){
					echo __('No slides found, please add at least one Slide Template to the choosen language.');
				}else{
					echo __('No slides found, please add some slides');
				}
				?>
			</div>
			<?php
        ...
        $do_static = $this->_framework->apply_filters('revslider_enable_static_layers', true);
        ...
        if($do_static){
            ...
            foreach($slides as $slide){
                ...
                if($staticID !== false){
                    $static_slide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
		...
		foreach($slides as $slide){
			...
			if($this->previewMode === false){ // do only if we are not in preview mode
				$ts = $this->_framework->current_time('timestamp');
			...
			if($isExternal != 'external'){
				...
				switch($alt_type){
					case 'media_library':
						$alt = $this->_framework->get_post_meta( $img_id, '_wp_attachment_image_alt', true );
					break;
					case 'file_name':
					...
					break;
					case 'custom':
						$alt = $this->_framework->esc_attr($slide->getParam('alt_attr', ''));
					break;
				}

                switch($title_type){
                    case 'media_library':
                        $img_title = $this->_framework->get_the_title($img_id);
                    break;
                    case 'file_name':
                        $imageFilename = $slide->getImageFilename();
                        $info = pathinfo($imageFilename);
                    ...
                    break;
                    case 'custom':
                        $img_title = $this->_framework->esc_attr($slide->getParam('title_attr', ''));
                    break;
                }
				...
				if($img_id !== false){
					$img_data = $this->_framework->wp_get_attachment_metadata( $img_id );
                ...
                $urlSlideImage = $this->_framework->esc_url($slide->getParam('slide_bg_external', ''));
				$alt = $this->_framework->esc_attr($slide->getParam('alt_attr', ''));
                $img_title = $this->_framework->esc_attr($slide->getParam('title_attr', ''));
				...
				if($thumb_do == 'slider'){ //use the slider settings for width / height
					...
					if($source_type == 'youtube' || $source_type == 'vimeo' ||
						...
						if(empty($urlThumb)){	//try to get resized thumb
							...
							$urlThumb = $this->_images->rev_aq_resize($url_img_link, $th_width, $th_height, true, true, true);
						}else{
							$urlThumb = $this->_images->rev_aq_resize($urlThumb, $th_width, $th_height, true, true, true);
							if(empty($urlThumb)){
			...
			if($enableLink == 'true'){
				...
				switch($linkType){
					...
					default:
					case 'regular':
                        $link = $this->_framework->do_shortcode($slide->getParam('link', ''), $slide);
			...
			$styleImage = '';
			$urlImageTransparent = Framework::$RS_PLUGIN_URL.'public/assets/images/transparent.png';
			...
			if($lazyLoad != 'none'){
				...
				$urlSlideImage = Framework::$RS_PLUGIN_URL.'public/assets/images/dummy.png';
			}
			...
			if($post_based_slider){//check if we are post based or normal slider
				$the_post = $this->_framework->get_post($slide->getID());
				$new_title = @$this->_framework->get_the_title($slide->getID());
				$the_excerpt = strip_tags($this->_framework->strip_shortcodes($the_post->post_excerpt));

				$slide_title = ' data-title="'.stripslashes($this->_framework->esc_attr($new_title)).'"';
				$slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), $this->_framework->esc_attr($the_excerpt)).'"';
			}else{
				$slide_title = ' data-title="'.stripslashes($this->_framework->esc_attr($slide->getParam("title","Slide"))).'"';
				$slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), $this->_framework->esc_attr($slide->getParam('slide_description', ''))).'"';
			}

			$slide_id = $slide->getID();
			if($slide->getParam('slide_id','') !== '') $slide_id = $this->_framework->esc_attr($slide_id);
			...
			for($mi=1;$mi<=10;$mi++){
				...
				if($pa !== ''){
                    $pa_limit = $slide->getParam('params_'.$mi.'_chars',10,RevSliderSlider::FORCE_NUMERIC);
					$pa = strip_tags($pa);
					$pa = mb_substr($pa, 0, $pa_limit, 'utf-8');
				}
                $add_params .= ' data-param'.$mi.'="'.stripslashes($this->_framework->esc_attr($pa)).'"';
			}
            ...
            $this->_framework->do_action('revslider_add_li_data', $this->slider, $slide);
			...
			$this->_framework->do_action('revslider_add_layer_html', $this->slider, $slide);
        ...
        if($do_static && !empty($this->static_slide)){
            ...
            if(!empty($layers)){
                ...
                $this->_framework->do_action('revslider_add_static_layer_html', $this->slider);
		...
		switch($videoType){
			...
			case 'twitter':
				...
				if($youtube_id !== ''){
					...
					$arguments.=';origin=' . rtrim($this->_framework->home_url(), '/') . ';';
					...
					if(!empty($html_mpeg)) $add_data .= '			data-videomp4="'.$this->_images->imageUrl($html_mpeg).'"'." \n";
			...
			case 'html5':
				...
				$html_mpeg = $this->_images->imageUrl( $slide->getParam('slide_bg_html_mpeg', '') );
				$html_webm = $this->_images->imageUrl( $slide->getParam('slide_bg_html_webm', '') );
				$html_ogv =  $this->_images->imageUrl( $slide->getParam('slide_bg_html_ogv', '')  );
				...
				if(!empty($html_ogv)) $add_data .= '			data-videoogv="'.$this->_images->imageUrl($html_ogv).'"'." \n";
				if(!empty($html_webm)) $add_data .= '			data-videowebm="'.$this->_images->imageUrl($html_webm).'"'." \n";
				if(!empty($html_mpeg)) $add_data .= '			data-videomp4="'.$this->_images->imageUrl($html_mpeg).'"'." \n";
	...
	private function putCreativeLayer($static_slide = false){
        ...
        $layers = $this->_framework->apply_filters('revslider_putCreativeLayer', $this->layers, $this, $static_slide);
    ...
    public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false){
        $layer = $this->_framework->apply_filters('revslider_putLayer_pre', $layer, $this, $row_group_uid, $static_slide, $special_type);
        ...
        $dbot = false;
			...
			**/
			$do_remove_inline = $this->_framework->apply_filters('revslider_remove_inline', true);
		...
		switch($type){
			...
			case 'column':
				...
				if($urlImage !== ''){
					...
					$objlib = new RevSliderObjectLibrary($this->_filesystem, $this->_framework, $this->_curl, $this->_images);
		...
		switch($type){
				...
				case 'button':
					...
					$html = $text;
					$html = $this->_framework->do_shortcode(stripslashes($html));
                    $html_toggle = $text_toggle;
                    $html_toggle = $this->_framework->do_shortcode(stripslashes($html_toggle));
				...
				case 'image':
					...
					if($img_size !== 'full'){
						if($cur_img_id !== false && !empty($cur_img_id)){
							$urlImage = $this->_framework->wp_get_attachment_image_src($cur_img_id, $img_size);
							$urlImage = $urlImage[0];
						}
					}
					...
					if($cur_img_id !== false && !empty($cur_img_id)){
						$img_data = $this->_framework->wp_get_attachment_metadata( $cur_img_id );
						if($img_data !== false && !empty($img_data)){
							if($img_size !== 'full'){
								if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
					...
					switch($alt_option){
						case 'media_library':
							if($cur_img_id !== false){
								$alt = $this->_framework->get_post_meta( $cur_img_id, '_wp_attachment_image_alt', true );
							}
							break;
						case 'file_name':
					...
					if($lazyLoad != 'none' || $do_ll == 'force' && $do_ll !== 'ignore'){
						...
						if($seo_opti === 'false' || $seo_opti === false){
							...
							$urlImage = Framework::$RS_PLUGIN_URL."public/assets/images/dummy.png";
						}else{
                ...
                case 'audio':
                    ...
                    $urlAudio = $this->_framework->esc_attr(RevSliderFunctions::getVal($videoData, 'urlAudio'));
                    if(!empty($urlAudio)) $add_data .= ' data-videomp4="'.$this->_images->imageUrl($urlAudio).'"';
				...
				case 'video':
					...
					switch($videoType){
						...
						case 'streaminstagramboth':
						case 'html5':
							$urlPoster = $this->_images->imageUrl( RevSliderFunctions::getVal($videoData, "urlPoster") );
							$urlMp4 =    $this->_images->imageUrl( RevSliderFunctions::getVal($videoData, "urlMp4")    );
							$urlWebm =   $this->_images->imageUrl( RevSliderFunctions::getVal($videoData, "urlWebm")   );
							$urlOgv =    $this->_images->imageUrl( RevSliderFunctions::getVal($videoData, "urlOgv")    );
							...
							$urlPoster = $this->_images->imageUrl($urlPoster);
							...
							if(!empty($urlMp4)) $add_data .= ' data-videomp4="'.$this->_images->imageUrl($urlMp4).'"';
					...
					$videoThumbnail = (isset($videoData["previewimage"])) ? $this->_images->imageUrl($videoData["previewimage"]) : '';
			...
			case 'column':
				...
				$add_data .= ' data-columnwidth="'.$this->_framework->esc_attr($size).'%"'."\n";
		...
		switch($this->container_mode){
    		...
    		if($has_trigger){
				...
				$last_trigger_state = '			data-lasttriggerstate="'.$this->_framework->esc_attr($trigger_memory).'"';
				...
				foreach($a_action as $num => $action){
					...
					switch($action){
						...
						case 'link':
							...
							//a_image_link
                            if(isset($a_image_link[$num])) $a_image_link[$num] = $this->_framework->do_shortcode($a_image_link[$num]);
						...
						break;
                        case 'add_to_cart':
                            $productData = $slide->getPostData();
                            $a_events[] = array(
                                'event' => $a_tooltip_event[$num],
                                'action' => 'callback',
                                'delay' => $a_action_delay[$num],
                                'callback' => $productData['add_to_cart_action_callback']
                            );
                        break;
					}

					// Filter the Actions
					$a_events = $this->_framework->apply_filters( 'rs_action_output_layer_action', $a_events, $action, $all_actions, $num, $this->slide);
					if(!isset($html_simple_link)){
						$html_simple_link = "";
					}
					$html_simple_link = $this->_framework->apply_filters('rs_action_output_layer_simple_link', $html_simple_link, $action, $all_actions, $num, $this->slide, $this->slider, $a_events);
		...
		echo ($tabindex != '') ? '			'.$tabindex." \n" : '';
			...
			echo '            data-type="'.$this->_framework->esc_attr($type).'"'." \n";
			...
			$this->_framework->do_action('revslider_add_layer_attributes', $layer, $this->slide, $this->slider);

			echo $a_html;

            // SVG OUTPUT
			if (!empty($svg_val) && is_array($svg_val) && sizeof($svg_val)>0){
				echo '            data-svg_src="'.$this->_framework->convertAssetUrlForOutput($svg_val['src']).'"'." \n";
		...
		if($special_type === false){
			echo $this->_framework->apply_filters('revslider_layer_content', stripslashes($html), $html, $this->slider->getID(), $this->slide, $layer)." ";
		}
		if ($toggle_allow!=false) {
			echo '</div>';
			echo '<div class="';
	...
	public function set_placeholders($text){
		if(isset($post->ID)) $text = str_replace(array('%current_page_link%', '{{current_page_link}}'), $this->_framework->get_permalink($post->ID), $text);
		$text = str_replace(array('%home_url%', '{{home_url}}'), $this->_framework->esc_url( $this->_framework->home_url( '/' ) ), $text);
        if(isset($post->ID)) $text = $this->_framework->apply_filters( 'revslider_gallery_set_placeholders', $text , $post->ID );
		return $text;
	}
	...
	private function putJS($markup_export = false){
		...
		$hideSliderAtLimit = $this->slider->getParam("hide_slider_under","0",RevSliderSlider::VALIDATE_NUMERIC);
		if(!empty($hideSliderAtLimit))
			$hideSliderAtLimit++;

		$hideCaptionAtLimit = $this->slider->getParam("hide_defined_layers_under","0",RevSliderSlider::VALIDATE_NUMERIC);;
		if(!empty($hideCaptionAtLimit))
			$hideCaptionAtLimit++;

		$hideAllCaptionAtLimit = $this->slider->getParam("hide_all_layers_under","0",RevSliderSlider::VALIDATE_NUMERIC);;
        ...
        $swipe_velocity = $this->slider->getParam("swipe_velocity","0.7",RevSliderSlider::VALIDATE_NUMERIC);
        $swipe_min_touches = $this->slider->getParam("swipe_min_touches","1",RevSliderSlider::VALIDATE_NUMERIC);
		...
		$do_delay = $this->_framework->apply_filters('revslider_add_js_delay', $do_delay);
        ...
        <script>
			...
			if (_RSdisableOnMobile && _RSisMobile) {
			    ...
			    [
                    ...
                    <?php $this->_framework->do_action('revslider_frontend_require', $this->slider, $this->sliderHtmlID); ?>
                ],
				...
				if (setREVStartSize!==undefined) setREVStartSize(
                    ...
                    if($this->slider->getParam("slider_type") == "fullscreen"){
                        echo ",";
                        echo " fullScreenAutoWidth:'". $this->_framework->esc_attr($this->slider->getParam("autowidth_force","off")) ."',";
                        echo " fullScreenAlignForce:'". $this->_framework->esc_attr($this->slider->getParam("full_screen_align_force","off")) ."',";
                        echo " fullScreenOffsetContainer:'". $this->_framework->esc_attr($this->slider->getParam("fullscreen_offset_container","")) ."',";
                        echo " fullScreenOffset:'". $this->_framework->esc_attr($this->slider->getParam("fullscreen_offset_size","")) ."'";
                    }
			...
			if($do_delay > 0){
				echo '						startDelay: '. $this->_framework->esc_attr($do_delay) .','."\n";
			}

			echo '						sliderType:"'. $this->_framework->esc_attr($slider_type) .'",'."\n";

			echo 'jsFileLocation:"'. $this->_framework->esc_attr($this->_framework->getAssetUrl('public/assets/js').'/') .'",'."\n";
			...
			echo '						sliderLayout:"'.$sl_layout.'",'."\n";
			echo '						dottedOverlay:"'. $this->_framework->esc_attr($this->slider->getParam("background_dotted_overlay","none")).'",'."\n";
			echo '						delay:'.$this->_framework->esc_attr($this->slider->getParam("delay","9000",RevSliderSlider::FORCE_NUMERIC)) .','."\n";

            $this->_framework->do_action('revslider_fe_javascript_option_output', $this->slider);
			...
			$rs_nav = new RevSliderNavigation($this->_framework, $this->_query);
            ...
            if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')){
				echo '						navigation: {'."\n";
				echo '							keyboardNavigation:"'. $this->_framework->esc_attr($keyboard_enabled) .'",'."\n";
				echo '							keyboard_direction: "'. $this->_framework->esc_attr($keyboard_direction) .'",'."\n";
				echo '							mouseScrollNavigation:"'. $this->_framework->esc_attr($mousescroll_enabled) .'",'."\n";
                echo '                             mouseScrollReverse:"'. $this->_framework->esc_attr($mousescroll_reverse) .'",'."\n";

				if($slider_type !== 'hero')
					echo '							onHoverStop:"'. $this->_framework->esc_attr($this->slider->getParam("stop_on_hover","on")).'",'."\n";

				$add_comma = false;
				if($touch_enabled == 'on'){
					$add_comma = true;
					echo '							touch:{'."\n";
					echo '								touchenabled:"'. $this->_framework->esc_attr($touch_enabled).'",'."\n";
                    echo '                    touchOnDesktop:"'. $this->_framework->esc_attr($touch_enabled_desktop).'",'."\n";
					echo '								swipe_threshold: '. $this->_framework->esc_attr($swipe_velocity) .','."\n";
					echo '								swipe_min_touches: '. $this->_framework->esc_attr($swipe_min_touches) .','."\n";
					echo '								swipe_direction: "'. $this->_framework->esc_attr($swipe_direction) .'",'."\n";
				...
				if($enable_arrows == 'on'){
					...
					$add_comma = true;
					echo '							arrows: {'."\n";
					echo '								style:"'. $this->_framework->esc_attr($navigation_arrow_style).'",'."\n";
					echo '								enable:';
					...
					if($hide_arrows_on_mobile === 'true') {
						echo '								hide_under:'. $this->_framework->esc_attr($this->slider->getParam('arrows_under_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					if($hide_arrows_over === 'true') {
						echo '								hide_over:'. $this->_framework->esc_attr($this->slider->getParam('arrows_over_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								hide_onleave:'.$arrows_always_on.','."\n";
					if($arrows_always_on === 'true'){
						echo '								hide_delay:'. $this->_framework->esc_attr($this->slider->getParam('hide_arrows','200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
						echo '								hide_delay_mobile:'. $this->_framework->esc_attr($this->slider->getParam('hide_arrows_mobile','1200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $arr_tmp);
					echo '\','."\n";
					...
					echo '									h_align:"'. $this->_framework->esc_attr($this->slider->getParam('leftarrow_align_hor','left')) .'",'."\n";
					echo '									v_align:"'. $this->_framework->esc_attr($this->slider->getParam('leftarrow_align_vert','center')) .'",'."\n";
					echo '									h_offset:'. $this->_framework->esc_attr($this->slider->getParam('leftarrow_offset_hor','20',RevSliderSlider::FORCE_NUMERIC)) .','."\n";
                    echo '                                    v_offset:'. $this->_framework->esc_attr($this->slider->getParam('leftarrow_offset_vert','0',RevSliderSlider::FORCE_NUMERIC))."\n";
					echo '								},'."\n";
					echo '								right: {'."\n";
                    echo (in_array($this->slider->getParam('rightarrow_position', 'slider'),array('layergrid','grid'))) ? '                                    container:"layergrid",'."\n" : '';
					echo '									h_align:"'. $this->_framework->esc_attr($this->slider->getParam('rightarrow_align_hor','right')).'",'."\n";
					echo '									v_align:"'. $this->_framework->esc_attr($this->slider->getParam('rightarrow_align_vert','center')).'",'."\n";
					echo '									h_offset:'. $this->_framework->esc_attr($this->slider->getParam('rightarrow_offset_hor','20',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                                    v_offset:'. $this->_framework->esc_attr($this->slider->getParam('rightarrow_offset_vert','0',RevSliderSlider::FORCE_NUMERIC))."\n";
					echo '								}'."\n";
					echo '							}'."\n";
				}
				...
				if($enable_bullets == 'on'){
					...
					if($hide_bullets_on_mobile === 'true'){
						echo '								hide_under:'. $this->_framework->esc_attr($this->slider->getParam('bullets_under_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					if($hide_bullets_over === 'true'){
						echo '								hide_over:'. $this->_framework->esc_attr($this->slider->getParam('bullets_over_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								style:"'. $this->_framework->esc_attr($navigation_bullets_style).'",'."\n";
					echo '								hide_onleave:'.$bullets_always_on.','."\n";
					if($bullets_always_on === 'true'){
						echo '								hide_delay:'. $this->_framework->esc_attr($this->slider->getParam('hide_bullets','200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
						echo '								hide_delay_mobile:'. $this->_framework->esc_attr($this->slider->getParam('hide_bullets_mobile','1200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								direction:"'. $this->_framework->esc_attr($this->slider->getParam('bullets_direction','horizontal')).'",'."\n";
                    echo (in_array($this->slider->getParam('bullets_position', 'slider'),array('layergrid','grid'))) ? '                                    container:"layergrid",'."\n" : '';
					echo '								h_align:"'. $this->_framework->esc_attr($this->slider->getParam('bullets_align_hor','right')).'",'."\n";
					echo '								v_align:"'. $this->_framework->esc_attr($this->slider->getParam('bullets_align_vert','center')).'",'."\n";
					echo '								h_offset:'. $this->_framework->esc_attr($this->slider->getParam('bullets_offset_hor','20',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								v_offset:'. $this->_framework->esc_attr($this->slider->getParam('bullets_offset_vert','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                                space:'. $this->_framework->esc_attr($this->slider->getParam('bullets_space','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $bul_tmp);
					echo '\''."\n";
					echo '							}'."\n";
				...
				if($enable_thumbnails == 'on'){
					...
					$add_comma = true;
					echo '							thumbnails: {'."\n";
					echo '								style:"'. $this->_framework->esc_attr($thumbnails_style).'",'."\n";
					echo '								enable:';
					echo ($this->slider->getParam('enable_thumbnails','off') == 'on') ? 'true' : 'false';
					echo ','."\n";
                    echo ($this->slider->getParam('rtl_thumbnails','off') == 'on') ? '                                rtl:true,'."\n" : '';
					echo '								width:'. $this->_framework->esc_attr($this->slider->getParam('thumb_width','100',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								height:'. $this->_framework->esc_attr($this->slider->getParam('thumb_height','50',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								min_width:'. $this->_framework->esc_attr($this->slider->getParam('thumb_width_min','100',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								wrapper_padding:'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_padding','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                    wrapper_color:"'.TPColorpicker::convert($this->_framework->esc_attr($this->slider->getParam('thumbnails_wrapper_color','transparent')),$this->slider->getParam('thumbnails_wrapper_opacity',false)).'",'."\n";
                    echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $thumbs_tmp);
					echo '\','."\n";
					echo '								visibleAmount:'. $this->_framework->esc_attr($this->slider->getParam('thumb_amount','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								hide_onmobile:'.$hide_thumbs_on_mobile.','."\n";
					if($hide_thumbs_on_mobile === 'true'){
						echo '								hide_under:'. $this->_framework->esc_attr($this->slider->getParam('thumbs_under_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					if($hide_thumbs_over === 'true'){
						echo '								hide_over:'. $this->_framework->esc_attr($this->slider->getParam('thumbs_over_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								hide_onleave:'.$thumbs_always_on.','."\n";
					if($thumbs_always_on === 'true'){
						echo '								hide_delay:'. $this->_framework->esc_attr($this->slider->getParam('hide_thumbs','200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
						echo '								hide_delay_mobile:'. $this->_framework->esc_attr($this->slider->getParam('hide_thumbs_mobile','1200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								direction:"'. $this->_framework->esc_attr($this->slider->getParam('thumbnail_direction','horizontal')).'",'."\n";
					echo '								span:';
					echo ($this->slider->getParam('span_thumbnails_wrapper','off') == 'on') ? 'true' : 'false';
					echo ','."\n";
					echo '								position:"'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_inner_outer','inner')).'",'."\n";
					...
					echo '								space:'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_space','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								h_align:"'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_align_hor','left')).'",'."\n";
					echo '								v_align:"'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_align_vert','center')).'",'."\n";
					echo '								h_offset:'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_offset_hor','20',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                                v_offset:'. $this->_framework->esc_attr($this->slider->getParam('thumbnails_offset_vert','0',RevSliderSlider::FORCE_NUMERIC))."\n";
					echo '							}'."\n";
				}
				if($enable_tabs == 'on'){
					...
					$add_comma = true;
					echo '							tabs: {'."\n";
					echo '								style:"'. $this->_framework->esc_attr($tabs_style).'",'."\n";
					echo '								enable:';
					echo ($this->slider->getParam('enable_tabs','off') == 'on') ? 'true' : 'false';
					echo ','."\n";
                    echo ($this->slider->getParam('rtl_tabs','off') == 'on') ? '                                rtl:true,'."\n" : '';
					echo '								width:'. $this->_framework->esc_attr($this->slider->getParam('tabs_width','100',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								height:'. $this->_framework->esc_attr($this->slider->getParam('tabs_height','50',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								min_width:'. $this->_framework->esc_attr($this->slider->getParam('tabs_width_min','100',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								wrapper_padding:'. $this->_framework->esc_attr($this->slider->getParam('tabs_padding','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                    wrapper_color:"'.TPColorpicker::convert($this->_framework->esc_attr($this->slider->getParam('tabs_wrapper_color','transparent')),$this->slider->getParam('tabs_wrapper_opacity',false)).'",'."\n";
                    echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $tabs_tmp);
					echo '\','."\n";
					echo '								visibleAmount: '. $this->_framework->esc_attr($this->slider->getParam('tabs_amount','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								hide_onmobile: '.$hide_tabs_on_mobile.','."\n";
					if($hide_tabs_on_mobile === 'true'){
						echo '								hide_under:'. $this->_framework->esc_attr($this->slider->getParam('tabs_under_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					if($hide_tabs_over === 'true'){
						echo '								hide_over:'. $this->_framework->esc_attr($this->slider->getParam('tabs_over_hidden','0',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								hide_onleave:'.$tabs_always_on.','."\n";
					if($tabs_always_on === 'true'){
						echo '								hide_delay:'. $this->_framework->esc_attr($this->slider->getParam('hide_tabs','200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
						echo '								hide_delay_mobile:'. $this->_framework->esc_attr($this->slider->getParam('hide_tabs_mobile','1200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					}
					echo '								hide_delay:'. $this->_framework->esc_attr($this->slider->getParam('hide_tabs','200',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								direction:"'. $this->_framework->esc_attr($this->slider->getParam('tabs_direction','horizontal')).'",'."\n";
					echo '								span:';
					echo ($this->slider->getParam('span_tabs_wrapper','off') == 'on') ? 'true' : 'false';
					echo ','."\n";
					echo '								position:"'. $this->_framework->esc_attr($this->slider->getParam('tabs_inner_outer','inner')).'",'."\n";
					...
					echo '								space:'. $this->_framework->esc_attr($this->slider->getParam('tabs_space','5',RevSliderSlider::FORCE_NUMERIC)).','."\n";
					echo '								h_align:"'. $this->_framework->esc_attr($this->slider->getParam('tabs_align_hor','left')).'",'."\n";
					echo '								v_align:"'. $this->_framework->esc_attr($this->slider->getParam('tabs_align_vert','center')).'",'."\n";
					echo '								h_offset:'. $this->_framework->esc_attr($this->slider->getParam('tabs_offset_hor','20',RevSliderSlider::FORCE_NUMERIC)).','."\n";
                    echo '                                v_offset:'. $this->_framework->esc_attr($this->slider->getParam('tabs_offset_vert','0',RevSliderSlider::FORCE_NUMERIC))."\n";
					echo '							}'."\n";
				}
				echo '						},'."\n";
			...
			if($slider_type == 'carousel'){
				...
				$car_space = $this->slider->getParam('carousel_space',0,RevSliderSlider::FORCE_NUMERIC);
				$car_stretch = $this->slider->getParam('carousel_stretch','off');
				$car_all_layers = $this->slider->getParam('showalllayers_carousel','off');
				$car_maxitems = $this->slider->getParam('carousel_maxitems',5,RevSliderSlider::FORCE_NUMERIC);
				...
				$car_speed = $this->slider->getParam('carousel_speed','800',RevSliderSlider::FORCE_NUMERIC);
				...
				$car_maxrotation = $this->slider->getParam('carousel_maxrotation',90,RevSliderSlider::FORCE_NUMERIC);
				...
				$car_scaledown = $this->slider->getParam('carousel_scaledown',50,RevSliderSlider::FORCE_NUMERIC);
				if($car_scaledown > 100) $car_scaledown = 100;

				$car_borderr = $this->slider->getParam('carousel_borderr',0,RevSliderSlider::FORCE_NUMERIC);
				$car_borderr_unit = $this->slider->getParam('carousel_borderr_unit','px');

				$car_padding_top = $this->slider->getParam('carousel_padding_top',0,RevSliderSlider::FORCE_NUMERIC);
				$car_padding_bottom = $this->slider->getParam('carousel_padding_bottom',0,RevSliderSlider::FORCE_NUMERIC);

				echo '						carousel: {'."\n";
				if($car_borderr > 0){
					echo '							border_radius: "'. $this->_framework->esc_attr($car_borderr.$car_borderr_unit) .'",'."\n";
				}
				if($car_padding_top > 0){
					echo '							padding_top: "'. $this->_framework->esc_attr($car_padding_top) .'",'."\n";
				}
				if($car_padding_bottom > 0){
					echo '							padding_bottom: "'. $this->_framework->esc_attr($car_padding_bottom) .'",'."\n";
				}
				if($carousel_rotation == 'on'){
					echo '							maxRotation: '. $this->_framework->esc_attr($car_maxrotation).','."\n";
					echo '							vary_rotation: "'. $this->_framework->esc_attr($car_varyrotate).'",'."\n";
				}
				if($carousel_scale == 'on'){
					echo '							minScale: '. $this->_framework->esc_attr($car_scaledown).','."\n";
					echo '							vary_scale: "'. $this->_framework->esc_attr($car_varyscale).'",'."\n";
				}
				echo '							horizontal_align: "'. $this->_framework->esc_attr($car_hpos).'",'."\n";
				echo '							vertical_align: "'. $this->_framework->esc_attr($car_vpos).'",'."\n";
				echo '							fadeout: "'. $this->_framework->esc_attr($car_fadeout).'",'."\n";
				if($car_fadeout == 'on'){
					echo '							vary_fade: "'. $this->_framework->esc_attr($car_varyfade).'",'."\n";
				}
				echo '							maxVisibleItems: '. $this->_framework->esc_attr($car_maxitems).','."\n";
				echo '							infinity: "'. $this->_framework->esc_attr($car_inf).'",'."\n";
				echo '							space: '. $this->_framework->esc_attr($car_space).','."\n";
				echo '                            stretch: "'. $this->_framework->esc_attr($car_stretch).'",'."\n";
				echo '                             showLayersAllTime: "'.$this->_framework->esc_attr($car_all_layers).'",'."\n";
				echo '                             easing: "'.$this->_framework->esc_attr($car_easing).'",'."\n";
				echo '                             speed: "'.$this->_framework->esc_attr($car_speed).'"'."\n";
			...
			$viewport_area = $this->slider->getParam('viewport_area', 80, RevSliderSlider::FORCE_NUMERIC);
			if($label_viewport === 'on'){
				echo '						viewPort: {'."\n";
				echo '							enable:true,'."\n";
				echo '							outof:"'.$this->_framework->esc_attr($viewport_start).'",'."\n";
                echo '                            visible_area:"'.$this->_framework->esc_attr($viewport_area).'%",'."\n";
                echo '                            presize:';
			...
			echo '						lazyType:"'. $this->_framework->esc_attr($lazyLoad).'",'."\n";
			...
			if($minHeight > 0){
				...
				if($use_scrolleffect =='on') {
					...
					if($this->slider->getParam('fade_scrolleffect','off') !== 'off')
						echo '                            fade:"'. $this->_framework->esc_attr($this->slider->getParam('fade_scrolleffect','off')) .'",'."\n";

					if($this->slider->getParam('blur_scrolleffect','off') !== 'off')
						echo '                            blur:"'. $this->_framework->esc_attr($this->slider->getParam('blur_scrolleffect','off')) .'",'."\n";

					if($this->slider->getParam('grayscale_scrolleffect','off') !== 'off')
						echo '                            grayscale:"'. $this->_framework->esc_attr($this->slider->getParam('grayscale_scrolleffect','off')) .'",'."\n";


					if($this->slider->getParam('scrolleffect_maxblur','10') !== '10')
						echo '                            maxblur:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_maxblur','10')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_bg','off') !== 'off')
						echo '                            on_slidebg:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_bg','off')) .'",'."\n";


					if($this->slider->getParam('scrolleffect_layers','off') !== 'off')
						echo '                            on_layers:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_layers','off')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_parallax_layers','off') !== 'off')
						echo '                            on_parallax_layers:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_parallax_layers','off')) .'",'."\n";


					if($this->slider->getParam('scrolleffect_static_layers','off') !== 'off')
						echo '                            on_static_layers:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_static_layers','off')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_static_parallax_layers','off') !== 'off')
						echo '                            on_parallax_static_layers:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_static_parallax_layers','off')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_direction','both') !== 'both')
						echo '                            direction:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_direction','both')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_multiplicator','1.3') !== '1.3')
						echo '                            multiplicator:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_multiplicator','1.3')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_multiplicator_layers','1.3') !== '1.3')
						echo '                            multiplicator_layers:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_multiplicator_layers','1.3')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_tilt','30') !== '30')
						echo '                            tilt:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_tilt','30')) .'",'."\n";

					if($this->slider->getParam('scrolleffect_off_mobile','on') !== 'on')
						echo '                            disable_on_mobile:"'. $this->_framework->esc_attr($this->slider->getParam('scrolleffect_off_mobile','on')) .'",'."\n";
					echo '                        },'."\n";
				}

				if($use_parallax == 'on'){
				echo '						parallax: {'."\n";
				echo '							type:"'. $this->_framework->esc_attr($parallax_type) .'",'."\n";
				echo '							origo:"'. $this->_framework->esc_attr($parallax_origo) .'",'."\n";
				echo '							speed:'. $this->_framework->esc_attr($parallax_speed) .','."\n";
                echo '                          speedbg:'. $this->_framework->esc_attr($parallax_bg_speed) .','."\n";
                echo '                          speedls:'. $this->_framework->esc_attr($parallax_ls_speed) .','."\n";
				echo '							levels:['. $this->_framework->esc_attr($parallax_level) .'],'."\n";

				if ($parallax_type == '3D') {
					echo '							ddd_shadow:"'. $this->_framework->esc_attr($parallax_ddd_shadow) .'",'."\n";
					echo '							ddd_bgfreeze:"'. $this->_framework->esc_attr($parallax_ddd_bgfreeze) .'",'."\n";
					echo '							ddd_overflow:"'. $this->_framework->esc_attr($parallax_ddd_overflow) .'",'."\n";
					echo '							ddd_layer_overflow:"'. $this->_framework->esc_attr($parallax_ddd_layer_overflow) .'",'."\n";
					echo '							ddd_z_correction:'. $this->_framework->esc_attr($parallax_ddd_zcorrection) .','."\n";
				}
			...
			if ($use_parallax != 'on' || ($use_parallax == 'on' && $parallax_type !='3D'))
				echo '						shadow:'. $this->_framework->esc_attr($this->slider->getParam("shadow_type","2")) .','."\n";

			if($use_spinner == '-1'){
				echo '						spinner:"off",'."\n";
			}else{
				echo '						spinner:"spinner'. $this->_framework->esc_attr($use_spinner).'",'."\n";
			}

			if($slider_type !== 'hero'){
				echo '						stopLoop:"'. $this->_framework->esc_attr($stopSlider) .'",'."\n";
				echo '						stopAfterLoops:'. $this->_framework->esc_attr($stopAfterLoops) .','."\n";
				echo '						stopAtSlide:'. $this->_framework->esc_attr($stopAtSlide) .','."\n";
				echo '						shuffle:"'. $this->_framework->esc_attr($this->slider->getParam("shuffle","off")) .'",'."\n";
			}

			echo '						autoHeight:"'. $this->_framework->esc_attr($this->slider->getParam("auto_height", 'off')). '",'."\n";
			...
			echo '						hideThumbsOnMobile:"'. $this->_framework->esc_attr($hideThumbsOnMobile) .'",'."\n";
			echo '						hideSliderAtLimit:'. $this->_framework->esc_attr($hideSliderAtLimit) .','."\n";
			echo '						hideCaptionAtLimit:'. $this->_framework->esc_attr($hideCaptionAtLimit) .','."\n";
			echo '						hideAllCaptionAtLilmit:'. $this->_framework->esc_attr($hideAllCaptionAtLimit) .','."\n";

			if($slider_type !== 'hero'){
				$start_with_slide_enable = $this->slider->getParam('start_with_slide_enable', 'off');
				if($start_with_slide_enable == 'on'){
					echo '						startWithSlide:'. $this->_framework->esc_attr($startWithSlide).','."\n";
				}
			}
            ...
            if($this->slider->getParam('ignore_height_changes', 'off') !== 'off'){
                echo '                            ignoreHeightChanges:"'. $this->_framework->esc_attr($this->slider->getParam('ignore_height_changes', 'off')).'",'."\n";
                echo '                            ignoreHeightChangesSize:'. intval($this->_framework->esc_attr($this->slider->getParam('ignore_height_changes_px', '0'))).','."\n";
            }

			echo '							simplifyAll:"'. $this->_framework->esc_attr($this->slider->getParam('simplify_ie8_ios4', 'off')).'",'."\n";

			if($slider_type !== 'hero')
				echo '							nextSlideOnWindowFocus:"'. $this->_framework->esc_attr($this->slider->getParam('next_slide_on_window_focus', 'off')).'",'."\n";
			...
			echo '    ';
            $this->_framework->do_action('revslider_fe_javascript_output', $this->slider, $this->sliderHtmlID);
			echo ''."\n";
			?>
		...
		if($js_to_footer && $this->previewMode == false && $markup_export == false){
            ...
            $this->_framework->add_action('wp_print_footer_scripts', array($this, 'add_inline_js'), 100);
		}
		...
		switch($use_spinner){
			case '1':
			case '2':
				if(!$this->_framework->is_admin()){?><script>
    ...
    public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array()){
		...
		try{
			$slver = $this->_framework->apply_filters('revslider_remove_version', RevSliderGlobals::SLIDER_REVISION);

			self::$sliderSerial++;

			$this->slider = new RevSliderSlider($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
			if($sliderID !== '-99'){
            ...
            $this->_framework->do_action('revslider_modify_core_settings', $this->slider);
			...
			$bannerWidth = $this->slider->getParam("width",null,RevSliderSlider::VALIDATE_NUMERIC,"Slider Width");
			$bannerHeight = $this->slider->getParam("height",null,RevSliderSlider::VALIDATE_NUMERIC,"Slider Height");
			...
			$backgroundColor = $this->_framework->esc_attr(trim($this->slider->getParam('background_color')));
            if(!empty($backgroundColor)) {
                $containerStyle .= 'background:'.TPColorpicker::get($backgroundColor).';';
            }

			//set padding
			$containerStyle .= 'padding:'.$this->_framework->esc_attr($this->slider->getParam('padding','0')).'px;';

			//set margin:
			if($sliderType != 'fullscreen'){

				if($sliderPosition != 'center'){
					$containerStyle .= 'margin-left:'.$this->_framework->esc_attr($this->slider->getParam('margin_left', '0', RevSliderSlider::FORCE_NUMERIC)).'px;';
					$containerStyle .= 'margin-right:'.$this->_framework->esc_attr($this->slider->getParam('margin_right', '0', RevSliderSlider::FORCE_NUMERIC)).'px;';
				}

				$containerStyle .= 'margin-top:'.$this->_framework->esc_attr($this->slider->getParam('margin_top', '0', RevSliderSlider::FORCE_NUMERIC)).'px;';
				$containerStyle .= 'margin-bottom:'.$this->_framework->esc_attr($this->slider->getParam('margin_bottom', '0', RevSliderSlider::FORCE_NUMERIC)).'px;';
			}
			...
			if($showBackgroundImage == 'true' || $showBackgroundImage == 'on'){
				$backgroundImage = $this->_framework->esc_attr($this->slider->getParam('background_image'));
				$backgroundFit = $this->_framework->esc_attr($this->slider->getParam('bg_fit', $this->slider->getParam('def-background_fit', 'cover')));
				$backgroundRepeat = $this->_framework->esc_attr($this->slider->getParam('bg_repeat', $this->slider->getParam('def-bg_repeat', 'no-repeat')));
				$backgroundPosition = $this->_framework->esc_attr($this->slider->getParam('bg_position', $this->slider->getParam('def-bg_position', 'center center')));
			...
			$maxWidth = $this->slider->getParam('max_width', '0', RevSliderSlider::FORCE_NUMERIC);
            ...
            $progress_style = ' style="height: '.$this->_framework->esc_attr($progress_height).'px; background: '.TPColorpicker::convert($progressbar_color, $progress_opa).';"';
			...
			$add_alias = '';
			if($this->_framework->is_super_admin() || $this->_framework->is_admin_bar_showing()){
				if($this->_framework->current_user_can('edit_theme_options')){
					$add_alias = ' data-alias="'.$this->_framework->esc_attr($this->slider->getAlias()).'"';
				}
			}
