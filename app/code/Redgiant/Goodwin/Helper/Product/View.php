...
namespace Redgiant\Goodwin\Helper\Product;
...
class View extends \Magento\Catalog\Helper\Product\View
{
    ...
    public function initProductLayout(ResultPage $resultPage, $product, $params = null)
    {
        ...
        if(!$product_page_type)
            $product_page_type = $this->_scopeConfig->getValue('goodwin_settings/product/product_page_layout', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeManager->getStore()->getId());
        if(empty($product_page_type))
            $product_page_type = 'default';
        
        $pageConfig->addBodyClass('product-page-layout-'.$product_page_type);
