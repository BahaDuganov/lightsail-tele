<?php
namespace Redgiant\Megamenu\Block;
...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    protected $_current_category;
    
    public function __construct(
        \Magento\Framework\View\Element\Template\Context $context,
        \Magento\Catalog\Helper\Category $categoryHelper,
        \Redgiant\Megamenu\Helper\Data $helper,
        ...
        $this->_current_category = $objectManager->get('Magento\Framework\Registry')->registry('current_category');
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null)
    {
        ...
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            foreach($children as $child) {
                ...
                $rg_menu_hide_item = $cat_model->getData('rg_menu_hide_item');
                
                if (!$rg_menu_hide_item) {
                    $sub_children = $this->getActiveChildCategories($child);
                    
                    $rg_menu_cat_label = $cat_model->getData('rg_menu_cat_label');
                    $rg_menu_icon_img = $cat_model->getData('rg_menu_icon_img');
                    $rg_menu_font_icon = $cat_model->getData('rg_menu_font_icon');
                    ...
                    if($level == 1 && $rg_menu_icon_img) {
                        $html .= '<div class="menu-thumb-img"><a class="menu-thumb-link" href="'.$this->_categoryHelper->getCategoryUrl($child).'"><img src="' . $this->_helper->getBaseUrl().'catalog/category/' . $rg_menu_icon_img . '" alt="'.$child->getName().'"/></a></div>';
                    }
                    $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($child).'">';
                    if ($level > 1 && $rg_menu_icon_img)
                        $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $rg_menu_icon_img . '" alt="'.$child->getName().'"/>';
                    elseif($rg_menu_font_icon)
                        $html .= '<em class="menu-thumb-icon '.$rg_menu_font_icon.'"></em>';
                    $html .= '<span>'.$child->getName();
                    if($rg_menu_cat_label)
                        $html .= '<span class="cat-label cat-label-'.$rg_menu_cat_label.'">'.$this->_megamenuConfig['cat_labels'][$rg_menu_cat_label].'</span>';
                    $html .= '</span></a>';
                    if(count($sub_children) > 0) {
    ...
    public function getParentCategoryIds($category) {
        $parent_ids = array();
    ...
    public function getCurrentParentCategoryIds() {
        return $this->getParentCategoryIds($this->_current_category);
    }
    ...
    public function getMegamenuHtml()
    {
        ...
        $this->_megamenuConfig = $this->_helper->getConfig('rg_megamenu');
        ...
        foreach($categories as $category) {
            ...
            if(!$rg_menu_hide_item) {
                ...
                $rg_menu_icon_img = $cat_model->getData('rg_menu_icon_img');
                $rg_menu_font_icon = $cat_model->getData('rg_menu_font_icon');
                $rg_menu_cat_columns = $cat_model->getData('rg_menu_cat_columns');
                $rg_menu_float_type = $cat_model->getData('rg_menu_float_type');                

                if(!$rg_menu_cat_columns){
                    $rg_menu_cat_columns = 4;
                }
                
                $menu_type = $cat_model->getData('rg_menu_type');
                if(!$menu_type)
                ...
                $rg_menu_static_width = $cat_model->getData('rg_menu_static_width');
                if(!$rg_menu_static_width)
                    $rg_menu_static_width = $this->_megamenuConfig['general']['static_width'];
                if($menu_type=="staticwidth" && $rg_menu_static_width)
                    $custom_style = ' style="width: '.$rg_menu_static_width.';"';
                    
                $item_class = 'level0 ';
                $item_class .= $menu_type.' ';
                if(in_array($category->getId(), $this->getCurrentParentCategoryIds()))
                    $item_class .= 'active ';
                $menu_top_content = $cat_model->getData('rg_menu_block_top_content');
                $menu_left_content = $cat_model->getData('rg_menu_block_left_content');
                $menu_left_width = $cat_model->getData('rg_menu_block_left_width');
                if(!$menu_left_content || !$menu_left_width)
                    $menu_left_width = 0;
                $menu_right_content = $cat_model->getData('rg_menu_block_right_content');
                $menu_right_width = $cat_model->getData('rg_menu_block_right_width');
                if(!$menu_right_content || !$menu_right_width)
                    $menu_right_width = 0;
                $menu_bottom_content = $cat_model->getData('rg_menu_block_bottom_content');
                if($rg_menu_float_type)
                    $rg_menu_float_type = 'fl-'.$rg_menu_float_type.' ';
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content)))
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$item_class.$rg_menu_float_type.'">';
                if(count($children) > 0) {
                    $html .= '<div class="open-children-toggle"></div>';
                }
                $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($category).'" class="level-top">';
                if ($rg_menu_icon_img)
                    $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $rg_menu_icon_img . '" alt="'.$category->getName().'"/>';
                elseif($rg_menu_font_icon)
                    $html .= '<em class="menu-thumb-icon '.$rg_menu_font_icon.'"></em>';
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $rg_menu_cat_columns);
